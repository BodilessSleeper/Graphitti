...
void Connections911::setupConnections(Layout *layout, AllVertices *vertices,
                                      AllEdges *edges) {
  ...
  for (int srcVertex = 0; srcVertex < numVertices; srcVertex++) {
    ...
    for (int destVertex = 0; destVertex < numVertices; destVertex++) {
      ...
      LOG4CPLUS_DEBUG(fileLogger_, "Source: " << srcVertex
                                              << " Dest: " << destVertex
                                              << " Dist: " << dist);
...
void Connections911::loadParameters() {
  ParameterManager::getInstance().getIntByXpath("//connsPerVertex/text()",
                                                connsPerVertex_);
  ParameterManager::getInstance().getIntByXpath("//psapsToErase/text()",
                                                psapsToErase_);
  ParameterManager::getInstance().getIntByXpath("//respsToErase/text()",
                                                respsToErase_);
}
...
bool Connections911::erasePSAP(AllVertices &vertices, Layout *layout) {
  ...
  if (changesMade) {
    // This is here so that we don't delete the vertex if we can't find any
    // edges
...
string Connections911::changedEdgesToXML(bool added) {
  ...
  os << "<Matrix name=\"" << name << "\" type=\"complete\" rows=\""
     << changed.size() << "\" columns=\"3\" multiplier=\"1.0\">" << endl;
...
string Connections911::erasedVerticesToXML() {
  ...
  os << "<Matrix name=\"verticesDeleted\" type=\"complete\" rows=\"1\" "
        "columns=\""
