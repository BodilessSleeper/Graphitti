...
void ConnGrowth::setupConnections(Layout *layout, AllVertices *vertices,
                                  AllEdges *synapses) {
  int numVertices = Simulator::getInstance().getTotalVertices();
  radiiSize_ = numVertices;

  W_ =
      new CompleteMatrix(MATRIX_TYPE, MATRIX_INIT, numVertices, numVertices, 0);
  radii_ = new VectorMatrix(MATRIX_TYPE, MATRIX_INIT, 1, numVertices,
                            growthParams_.startRadius);
  rates_ = new VectorMatrix(MATRIX_TYPE, MATRIX_INIT, 1, numVertices, 0);
  delta_ =
      new CompleteMatrix(MATRIX_TYPE, MATRIX_INIT, numVertices, numVertices);
  area_ =
      new CompleteMatrix(MATRIX_TYPE, MATRIX_INIT, numVertices, numVertices, 0);
...
void ConnGrowth::loadParameters() {
  ParameterManager::getInstance().getBGFloatByXpath(
      "//GrowthParams/epsilon/text()", growthParams_.epsilon);
  ParameterManager::getInstance().getBGFloatByXpath(
      "//GrowthParams/beta/text()", growthParams_.beta);
  ParameterManager::getInstance().getBGFloatByXpath("//GrowthParams/rho/text()",
                                                    growthParams_.rho);
  ParameterManager::getInstance().getBGFloatByXpath(
      "//GrowthParams/targetRate/text()", growthParams_.targetRate);
  ParameterManager::getInstance().getBGFloatByXpath(
      "//GrowthParams/minRadius/text()", growthParams_.minRadius);
  ParameterManager::getInstance().getBGFloatByXpath(
      "//GrowthParams/startRadius/text()", growthParams_.startRadius);
  ...
  if (growthParams_.epsilon != 0) {
    ...
    LOG4CPLUS_FATAL(fileLogger_,
                    "Parameter GrowthParams/epsilon/ has a value of 0" << endl);
    exit(EXIT_FAILURE);
  }
}
...
void ConnGrowth::updateConns(AllVertices &vertices) {
  ...
  for (int i = 0; i < Simulator::getInstance().getTotalVertices(); i++) {
    ...
    (*rates_)[i] =
        spNeurons.spikeCount_[i] / Simulator::getInstance().getEpochDuration();
  }

  // compute vertex radii change and assign new values
  (*outgrowth_) = 1.0 - 2.0 / (1.0 + exp((growthParams_.epsilon -
                                          *rates_ / growthParams_.maxRate) /
                                         growthParams_.beta));
  (*deltaR_) = Simulator::getInstance().getEpochDuration() * growthParams_.rho *
               *outgrowth_;
  (*radii_) += (*deltaR_);
}
...
void ConnGrowth::updateFrontiers(const int numVertices, Layout *layout) {
  ...
  for (int unit = 0; unit < numVertices - 1; unit++) {
    for (int i = unit + 1; i < numVertices; i++) {
      (*delta_)(unit, i) =
          (*layout->dist_)(unit, i) - ((*radii_)[unit] + (*radii_)[i]);
    ...
    for (int j = 0; j < numVertices; j++) {
      ...
      if ((*delta_)(i, j) < 0) {
        ...
        if (lenAB + min(r1, r2) <= max(r1, r2)) {
          (*area_)(i, j) =
              pi * min(r1, r2) * min(r1, r2); // Completely overlapping unit

          LOG4CPLUS_DEBUG(fileLogger_,
                          "Completely overlapping (i, j, r1, r2, area): "
                              << i << ", " << j << ", " << r1 << ", " << r2
                              << ", " << (*area_)(i, j) << endl);
        } else {
          ...
          if (fabs(cosCBA) >= 1.0 || fabs(cosCAB) >= 1.0) {
            ...
            (*area_)(i, j) = 0.5 * (r22 * (angCBD - sin(angCBD)) +
                                    r12 * (angCAD - sin(angCAD)));
          }
        }
      }
    }
...
void ConnGrowth::updateSynapsesWeights(const int numVertices,
                                       AllVertices &vertices, AllEdges &iedges,
                                       Layout *layout) {
  ...
  for (int srcVertex = 0; srcVertex < numVertices; srcVertex++) {
    ...
    for (int destVertex = 0; destVertex < numVertices; destVertex++) {
      ...
      BGSIZE iEdg =
          Simulator::getInstance().getMaxEdgesPerVertex() * destVertex;
      for (BGSIZE synapseIndex = 0; synapse_adjusted < synapseCounts;
           synapseIndex++, iEdg++) {
