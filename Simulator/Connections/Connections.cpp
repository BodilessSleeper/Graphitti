...
Connections::Connections() {
  ...
  string type;
  ParameterManager::getInstance().getStringByXpath("//EdgesParams/@class",
                                                   type);
  ...
  function<void()> printParametersFunc =
      bind(&Connections::printParameters, this);
  ...
  OperationManager::getInstance().registerOperation(
...
shared_ptr<AllEdges> Connections::getEdges() const { return edges_; }
...
void Connections::createEdgeIndexMap() {
  ...
  if (synapseIndexMap_ == nullptr) {
    synapseIndexMap_ =
        shared_ptr<EdgeIndexMap>(new EdgeIndexMap(vertexCount, maxEdges));
  }
...
void Connections::updateSynapsesWeights(const int numVertices,
...
void Connections::createSynapsesFromWeights(const int numVertices,
                                            Layout *layout,
                                            AllVertices &vertices,
                                            AllEdges &synapses) {
  // for each neuron
  for (int i = 0; i < numVertices; i++) {
    ...
    for (BGSIZE synapseIndex = 0;
         synapseIndex < Simulator::getInstance().getMaxEdgesPerVertex();
         synapseIndex++) {
      BGSIZE iEdg =
          Simulator::getInstance().getMaxEdgesPerVertex() * i + synapseIndex;
      // if the synapse weight is not zero (which means there is a connection),
      // create the synapse
