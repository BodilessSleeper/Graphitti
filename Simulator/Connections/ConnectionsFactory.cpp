...
void ConnectionsFactory::registerClass(const string &className,
                                       CreateFunction function) {
  createFunctions[className] = function;
}

/// Creates concrete instance of the desired connections class.
shared_ptr<Connections>
ConnectionsFactory::createConnections(const string &className) {
  connectionsInstance =
      shared_ptr<Connections>(invokeCreateFunction(className));
  return connectionsInstance;
}

/// Create an instance of the connections class using the static ::Create()
/// method.
///
