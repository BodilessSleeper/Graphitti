...
///  them). Initialize the small world network characterized by parameters:
...
void ConnStatic::setupConnections(Layout *layout, AllVertices *vertices,
                                  AllEdges *edges) {
  Simulator &simulator = Simulator::getInstance();
  int numVertices = simulator.getTotalVertices();
  vector<DistDestVertex> distDestVertices[numVertices];
  BGSIZE maxTotalEdges =
      simulator.getMaxEdgesPerVertex() * simulator.getTotalVertices();
  ...
  for (int srcVertex = 0; srcVertex < numVertices; srcVertex++) {
    ...
    // sort ascendant
    sort(distDestVertices[srcVertex].begin(),
         distDestVertices[srcVertex].end());
    // pick the shortest connsPerVertex_ connections
    for (BGSIZE i = 0;
         i < distDestVertices[srcVertex].size() && (int)i < connsPerVertex_;
         i++) {
      ...
      // set edge weight
      // TODO: we need another synaptic weight distibution mode (normal
      // distribution)
...
void ConnStatic::loadParameters() {
  ParameterManager::getInstance().getBGFloatByXpath(
      "//threshConnsRadius/text()", threshConnsRadius_);
  ParameterManager::getInstance().getIntByXpath("//connsPerNeuron/text()",
                                                connsPerVertex_);
  ParameterManager::getInstance().getBGFloatByXpath(
      "//rewiringProbability/text()", rewiringProbability_);
  // ParameterManager::getInstance().getBGFloatByXpath("//excWeight/min/text()",
  // excWeight_[0]);
  // ParameterManager::getInstance().getBGFloatByXpath("//excWeight/max/text()",
  // excWeight_[1]);
  // ParameterManager::getInstance().getBGFloatByXpath("//inhWeight/min/text()",
  // inhWeight_[0]);
  // ParameterManager::getInstance().getBGFloatByXpath("//inhWeight/max/text()",
  // inhWeight_[1]);
}
